...
bool Map::is_solid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    if (tile == 81) return false;
    if (tile == 320) return false;
    if (tile == 321) return false;
    if (tile == 340) return false;
    if (tile == 341) return false;
    if (tile == 500) return false;
    if (tile == 501) return false;
    if (tile == 502) return false;
    if (tile == 163) return false;
    if (tile == 253) return false;
    if (tile == 233) return false;
    if (tile == 273) return false;
    if (tile == 293) return false;
    if (tile == 120) return false;
...
// In Map.cpp:
void Map::set_tile(int grid_x, int grid_y, unsigned int new_tile) {
    if (grid_x < 0 || grid_x >= m_width || grid_y < 0 || grid_y >= m_height) return;
    m_level_data[grid_y * m_width + grid_x] = new_tile;
}
